...
class BlockedConversationsAdapter(
    ...
    override fun actionItemPressed(id: Int) {
        ...
        when (id) {
            ...
            R.id.cab_unblock -> unblockConversation()
    ...
    private fun unblockConversation() {
        if (selectedKeys.isEmpty()) {
            return
        }

        ensureBackgroundThread {
            val conversationsToUnblock = currentList.filter { selectedKeys.contains(it.hashCode()) } as ArrayList<Conversation>
            conversationsToUnblock.forEach {
                activity.restoreAllMessagesFromBlockedForConversation(it.threadId)
            }

            removeConversationsFromList(conversationsToUnblock)
        }
    }
